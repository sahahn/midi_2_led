...
def input_main(values, device_id):
    pygame.init()
    pygame.midi.init()
        ...
        while going:
            ...
            if i.poll():
                ...
                if midi_events[0][0][0] == 153:
                     ...
                     if ((hit % 8) == 0):
                         for h in range(((NUM_LIGHTS * 3) + 1), (CHAN + 1), 3):
    ...
    while going:
		...
		lights[L][1] = velocity  
		  ...
		  values[(L*3)] = int((N / 127) * velocity)
                  ...
                  dmxSend(values) #Send out the new DMX information
		...
		stack.sort(reverse=True)
              ...
              if (mod == 0):
                dmxSend(values) #If the mod is set back to 0, re-send dmx, as error handling
...
def main():
    ...
    values = Array('B',range(CHAN + 2))

    for x in (3,5):
        Process(target=input_main, args=(values,x)).start()
