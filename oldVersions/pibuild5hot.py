...
def synth_main(device_id):
    ...
    while going:
		...
		lights[L][1] = velocity  
                ...
                N = ((key % COLOR_RANGE) * (360 / COLOR_RANGE)) #Set the pre-bend N value based on what note is played, and store it
                lights[L][0] = N
              ...
              for z in range(1,(NUM_LIGHTS + 1)):
                if (lights[z][1] != 0):
                  ...
                  if ((N >= 0) and (N < 50)):
                    ...
                    values[((z*3)-2)] = (2 * velocity)
		    values[((z*3)-1)] = 0
		    values[(z*3)] = int(((50-N) / 127) * velocity)
            ...
            if midi_events[0][0][0] == keyPress:
                 ...
                 if ((hit % 8) == 0):
                     for h in range(1,(CHAN + 1),3):
                         values[h] = velocity * 2
                         values[h+1] = 0
                         values[h+2] = int(velocity * .4)

                 elif ((hit % 8) == 1):
                     for h in range(1,(CHAN + 1),3):
                         values[h] = velocity * 2
                         values[h+1] = 0
                         values[h+2] = int(velocity * .04)

                 elif ((hit % 8) == 2):
                     for h in range(1,(CHAN + 1),3):
                         values[h] = velocity * 2
                         values[h+1] = int(velocity * .3)
                         values[h+2] = 

                 elif ((hit % 8) == 3):
                     for h in range(1,(CHAN + 1),3):
                         values[h] = velocity * 2
                         values[h+1] = int(velocity * .7)
                         values[h+2] = 0
                     ...
                     for h in range(1,(CHAN + 1),3):
                         values[h] = velocity * 2
                         values[h+1] = int(velocity * 1.4)
                         values[h+2] = 0

                 elif ((hit % 8) == 6):
                     for h in range(1,(CHAN + 1),3):
                         values[h] = velocity * 2
                         values[h+1] = int(velocity * 1.7)
                         values[h+2] = 0

                 elif ((hit % 8) == 7):
                     for h in range(1,(CHAN + 1),3):
                         values[h] = int(velocity * 1.9)
                         values[h+1] = velocity * 2
                         values[h+2] = 0
